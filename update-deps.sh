...
# Updates the vendored bundles in scripts/ (core-js, @dsplay/template-utils, jquery) to their
# latest published version, skipping (and warning about) major version bumps, which may
# contain breaking changes and need a human to review before applying.
#
# The jquery.easing / jquery.skitter plugins are NOT handled here - they're old, unmaintained
# third-party plugins not published to npm under a stable name, so they're a frozen vendored
...
# update_jquery: versioned filename (jquery-<version>.min.js), referenced in index.html
update_jquery() {
  local pkg="jquery"
  local current latest
  current=$(current_version "$pkg")
  ...
  if [ "$current" = "$latest" ]; then
    echo "jquery is up to date ($current)"
    return
  fi

  if [ -n "$current" ] && [ "$(major "$current")" != "$(major "$latest")" ]; then
    echo "jquery has a new MAJOR version available: $current -> $latest"
    echo "  Skipping - this may include breaking changes. Review https://github.com/jquery/jquery/releases"
    echo "  and update manually (see README.md) if you want it."
    return
  fi

  echo "Updating jquery: ${current:-none} -> $latest"
  curl -sL "https://code.jquery.com/jquery-$latest.min.js" -o "scripts/jquery-$latest.min.js"
  [ -n "$current" ] && [ -f "scripts/jquery-$current.min.js" ] && rm "scripts/jquery-$current.min.js"
  sed -i.bak -E "s#scripts/jquery-[0-9.]+\.min\.js#scripts/jquery-$latest.min.js#" index.html && rm index.html.bak
  set_version "$pkg" "$latest"
}
